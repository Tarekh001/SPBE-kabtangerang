...
import { Menu, X, ChevronDown, Globe, Info, BarChart3, Layers, Sun, Moon, Search } from "lucide-react";
...
export const Navbar = () => {
  ...
  /**
   * Handle domain dropdown click:
   * Scroll to indikator section + dispatch custom event to highlight domain
   */
  const handleDomainClick = (target, domain) => {
    ...
    // Dispatch custom event so Indikator component can react
    setTimeout(() => {
      window.dispatchEvent(
        new CustomEvent("spbe-domain-select", { detail: { domain } })
      );
    }, 400); // delay to allow scroll to complete
  };
  ...
  const handleSearchResultClick = (item) => {
    ...
    // If the item has a domain, also trigger the domain filter
    if (item.domain) {
      setTimeout(() => {
        window.dispatchEvent(
          new CustomEvent("spbe-domain-select", { detail: { domain: item.domain } })
        );
      }, 400);
    }
  };
  ...
  return (
    <header className="sticky top-0 z-50 w-full" ref={navRef}>
      <nav
        className={`gradient-hero text-primary-foreground transition-all duration-300 ${
          scrolled ? "shadow-elegant py-0" : "shadow-soft"
        }`}
      >
        <div className="container flex items-center justify-between h-14 gap-2">
          {/* Logo → scroll to beranda */}
          <button
            onClick={() => handleNavClick("beranda")}
            className="group flex items-center gap-2 font-display font-extrabold text-xl tracking-wide shrink-0"
          >
            ...
            <span className="hidden sm:inline">
              SPBE<span className="text-accent"> Tangerang</span>
            </span>
          </button>

          {/* Theme Toggle — right after logo */}
          <button
            onClick={toggleDarkMode}
            className="w-9 h-9 rounded-lg bg-white/15 backdrop-blur grid place-items-center hover:bg-white/25 transition-all duration-300 shrink-0"
            aria-label={darkMode ? "Switch to light mode" : "Switch to dark mode"}
            title={darkMode ? "Mode Terang" : "Mode Gelap"}
          >
            {darkMode ? (
              <Sun className="w-4 h-4 text-accent transition-transform duration-300 hover:rotate-180" />
            ) : (
              <Moon className="w-4 h-4 transition-transform duration-300 hover:-rotate-12" />
            )}
          </button>

          {/* Global Search Bar — after theme toggle */}
          <div className="relative flex-1 max-w-xs hidden md:block" ref={searchRef}>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Cari indikator, regulasi..."
                className="w-full pl-9 pr-4 py-1.5 rounded-lg bg-white/15 backdrop-blur border border-white/20 text-sm text-primary-foreground placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-accent/50 focus:bg-white/20 transition-all"
              />
            </div>
                ...
                {searchResults.length > 0 ? (
                  searchResults.map((item, idx) => (
                    <button
                      ...
                      className="w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-secondary hover:pl-6 transition-all duration-200 border-b border-border/50 last:border-0"
                    >
                      <Search className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
                      <div className="min-w-0">
                        <div className="font-medium truncate">{item.label}</div>
                        <div className="text-xs text-muted-foreground">
                          {item.domain ? `Domain: ${item.domain}` : `Section: ${item.section}`}
                        </div>
                      </div>
                    </button>
                  ))
                  ...
                  {it.dropdown && isOpen && (
                    <div className="absolute top-full left-0 mt-1 w-48 bg-card text-card-foreground rounded-xl shadow-elegant overflow-hidden animate-scale-in origin-top">
                      {it.dropdown.map((d) => (
                        <button
                          key={d.label}
                          onClick={() => handleDomainClick(d.target, d.domain)}
            ...
            <div className="container pt-3 pb-1">
              <div className="relative" ref={searchRef}>
                ...
                <input
                  ...
                  onFocus={() => setSearchOpen(true)}
                  placeholder="Cari indikator, regulasi..."
                  className="w-full pl-9 pr-4 py-2 rounded-lg bg-white/15 backdrop-blur border border-white/20 text-sm text-primary-foreground placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-accent/50 transition-all"
                />
                    ...
                    {searchResults.length > 0 ? (
                      searchResults.map((item, idx) => (
                        <button
                          key={idx}
                          onClick={() => handleSearchResultClick(item)}
                          className="w-full text-left flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-secondary transition-all border-b border-border/50 last:border-0"
                        >
                          <Search className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
